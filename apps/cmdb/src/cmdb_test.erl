    ...
    lists:foreach(fun(P) -> 
                          spawn_link(fun() ->
                                        ...
                                        lists:foreach(fun(It) ->
                                                              Entries = [{ N0, P, It, <<"plop">>} || N0 <- lists:seq(1, N)],
                                                              {T, _} = timer:tc(fun() ->
                                                                               cmdb:put(Name, Entries)
                                                                       end),
                                                              Collector ! {N, T}
                                                      end, lists:seq(1, I)),
...
loop(SoFar, Writers, Writers, Since) ->
    ...
    Speed = SoFar/Elapsed,
    cmkit:success({finished, #{ inserted => SoFar,
                                seconds => Elapsed, 
                                speed => trunc(Speed)}});
...
m(T) ->
    {Out, In} = ?MODULE:T(),
    {{S, P, O, _}, V2, Seen2, F2} = lists:foldl(fun(E, {{S0, P0, O0, H0}, V, Seen, F}) ->
                                                        cmdb_util:m2(E, S0, P0, O0, H0, V, Seen, F)
                                                end, {{n, n, n, n}, n, n, []}, In),
    ...
    case F3 of 
        Out -> 
            ok;
        Other ->
            cmkit:danger({cmdb_teest, m, T, #{ input => In,
                                              expected => Out,
                                              actual => Other }}),
            ok
    end.
