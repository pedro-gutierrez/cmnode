...
compile(#{ <<"type">> := <<"theme">> }=Spec) -> {ok, compile_theme(Spec)};
...
compile_encoders(Encs) when is_map(Encs) ->
    compile_encoders(maps:keys(Encs), Encs, #{});

compile_encoders(Other) ->
...
compile_updates(Updates) when is_map(Updates) -> 
    compile_updates(maps:keys(Updates), Updates, #{});

compile_updates(Other) -> 
